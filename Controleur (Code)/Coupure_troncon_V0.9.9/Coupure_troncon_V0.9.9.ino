    /* V0.9.9 - 10/03/2020 - Maxime Viaud    10kohm resistors(without internal pulldown)
    ...
    *  Manual mode available (don't need AOG to run)
...
void setup()
  ...
  while (Serial.available()==0){ //while nothing is received trough the USB, run fonction "Buttons2()" ... tant que l'USB ne reçoit rien du logiciel, on éxécute "Buttons2()"
   Buttons2();
   }     
}
...
void loop()
{
  ...
  if (Serial.available() > 7 && isDataFound) //si on à plus de 7 octets après l'en-tête, on met chaque octet dans une variable (octet 1 = relayHi, etc.).
  {
    isDataFound = false;
    relayHi = Serial.read();
  ...
  if(digitalRead(buttonboomonPin)== HIGH && digitalRead(buttonmodePin)== HIGH){ //if you are disconected from AOG, press "On" button and "Manual mode" button to switch to fully manual mode                                                           
    asm volatile ("  jmp 0");                                                   ////si vous êtes déconnectés d'AOG, pressez les bouttons "mode manuel" et "rampe on" pour passer en control totalement manuel 
  }                             //if the 2 buttons mentionned are pressed, the code restart to line 0    .... si les 2 boutons sont présés, alors le code redémarre à la ligne 0
...
digitalWrite(6, bitRead(sect,0));
digitalWrite(7, bitRead(sect,1));
digitalWrite(8, bitRead(sect,2));
digitalWrite(9, bitRead(sect,3));
digitalWrite(10, bitRead(sect,4));
digitalWrite(11, bitRead(sect,5));
digitalWrite(12, bitRead(sect,6));
digitalWrite(13, bitRead(sect,7));
}
